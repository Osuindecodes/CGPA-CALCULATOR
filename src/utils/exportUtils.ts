...
export const exportToCSV = (courses: Course[], summary: CGPASummaryData) => {
  ...
  rows.push(['CGPA ACADEMIC REPORT']);
  ...
  activeYears.forEach(yr => {
    ...
    ([1, 2] as const).forEach(sem => {
      ...
      rows.push([`Semester ${sem}`]);
      rows.push(['Course Name', 'Credit Units', 'Grade', 'Grade Points', 'Weighted Points']);
      semCourses.forEach(c => rows.push([
        c.name, c.creditUnits, c.grade,
        gradePoints[c.grade],
        c.creditUnits * gradePoints[c.grade],
      ]));
      rows.push([`Semester ${sem} GPA`, totalCredits, '', totalPoints, cgpa]);
      rows.push([]);
    });

    const { totalCredits: yc, totalPoints: yp, cgpa: ygpa } = calcGPA(yearCourses);
    rows.push([`Year ${yr} CGPA`, yc, '', yp, ygpa]);
    rows.push([]);
    rows.push([]);
  });

  rows.push(['--- CUMULATIVE CGPA ---']);
  rows.push(['Total Credits', 'Total Points', 'CGPA', 'Classification']);
  rows.push([summary.totalCredits, summary.totalPoints, summary.cgpa, getCGPAClass(summary.cgpa)]);

  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws['!cols'] = [{ wch: 40 }, { wch: 14 }, { wch: 10 }, { wch: 14 }, { wch: 16 }];
