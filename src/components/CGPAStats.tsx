...
const CGPAStats = () => {
  ...
  // ── Bar chart data: one bar per semester per year ──────────────────────────
  ...
  // ── Grade distribution ─────────────────────────────────────────────────────
  const gradeCounts = (['A','B','C','D','E','F'] as const).map(g => ({
    grade: g,
  ...
  const aCount = courses.filter(c => c.grade === 'A').length;
  const fCount = courses.filter(c => c.grade === 'F').length;

  return (
    <div className="space-y-6 animate-fade-in [animation-delay:700ms]">
      {/* ── Stat cards ── */}
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={barData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="gpa" fill={TEAL} radius={[4, 4, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                ...
                <Tooltip formatter={(v: number) => [v, 'courses']} contentStyle={{ fontSize: 12, borderRadius: 8 }} />
              </PieChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {yearRows.map(({ yr, cgpa, totalCredits, totalPoints, s1, s2 }) => (
                  <tr key={yr} className="border-b last:border-0 hover:bg-muted/40 transition-colors">
